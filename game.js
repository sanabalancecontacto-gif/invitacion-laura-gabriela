/* ==========================================================================
   ...
   Game Engine: Breakout / Rompe-Hielo, Particles, Web Audio & Navigation
   ========================================================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    snowballImg.src = './assets/Fluffy_white_snowball_with_ice_20260919111321.jpeg';

    const icebergImg = new Image();
    icebergImg.src = './assets/Glowing_jagged_iceberg_isolated_20260919111325.jpeg';

    const characterImg = new Image();
    characterImg.src = './assets/personaje.jpeg';

    let gameLoopId = null;
    let isGameRunning = false;

    // Game Variables
    let paddle = { x: 0, y: 0, width: 110, height: 26, speed: 8 };
    ...
    function resizeGameCanvas() {
        ...
        paddle.width = Math.min(130, gameCanvas.width * 0.28);
    }
        ...
        for (let r = 0; r < brickRows; r++) {
            for (let c = 0; c < brickCols; c++) {
                bricks.push({
                    ...
                    height: brickHeight,
                    active: true,
                    hitsRequired: (r === 0) ? 2 : 1, // Top row requires 2 hits
                    hitsLeft: (r === 0) ? 2 : 1
                });
            }
        }
    ...
    function initGame() {
        ...
        // Hide touch guide after 4 seconds
    ...
    function gameLoop() {
        ...
        if (bgImg.complete && bgImg.naturalWidth !== 0) {
            ...
            ctx.fillStyle = 'rgba(6, 17, 28, 0.4)';
        ...
        // Bottom collision (Paddle hit or reset)
        ...
        bricks.forEach(brick => {
            if (brick.active) {
                ctx.save();
                // Draw Iceberg image or stylish block
                if (icebergImg.complete && icebergImg.naturalWidth !== 0) {
                    ctx.drawImage(icebergImg, brick.x, brick.y, brick.width, brick.height);
                } else {
                ...
                // Ice glow border
                ctx.strokeStyle = brick.hitsLeft === 2 ? '#ffffff' : '#3a7bd5';
                ctx.lineWidth = 2;
        ...
        if (snowballImg.complete && snowballImg.naturalWidth !== 0) {
            ctx.drawImage(snowballImg, ball.x - ball.radius, ball.y - ball.radius, ball.radius * 2, ball.radius * 2);
        } else {
    ...
    // --- CONFETTI CANNON FOR CELEBRATION ---
